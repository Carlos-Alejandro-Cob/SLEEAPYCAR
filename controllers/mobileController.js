...
// API: Agregar envío a la ruta del repartidor con código del bodeguero
exports.agregarEnvioARuta = async (req, res) => {
    try {
        const { codigo_envio, codigo_confirmacion } = req.body;
        const id_usuario = req.user.id_usuario;

        if (!codigo_confirmacion || codigo_confirmacion.toString().trim() === '') {
            return res.status(400).json({
                success: false,
                message: 'Debe ingresar el código de confirmación que le dio el bodeguero.'
            });
        }

        if (!codigo_envio || codigo_envio.toString().trim() === '') {
            return res.status(400).json({
                success: false,
                message: 'Debe ingresar el código de envío.'
            });
        }

        const envios = await Envio.findAll({ q: codigo_envio.toString().trim(), estado: 'NO_ENTREGADO' });
        if (envios.length === 0) {
            return res.status(404).json({
                success: false,
        ...
        let validacion;
        try {
            validacion = await CodigoConfirmacion.validarYUsar(codigo_confirmacion.toString().trim(), envio._id, id_usuario);
        } catch (error) {
        ...
        if (validacion.tipo !== 'BODEGUERO_CHOFER') {
            return res.status(400).json({
                success: false,
                message: 'El código no es válido para asignar este envío.'
            });
        }
        ...
        await logger.logAction(id_usuario, 'ASIGNAR_RUTA_MOVIL', `Envío ${envio.ID_Envio} asignado a repartidor (app móvil)`);

        res.json({
            success: true,
            message: `Envío ${envio.ID_Envio} asignado a tu ruta.`,
            envio: {
                id: envio._id,
                codigo: envio.ID_Envio,
                destinatario: envio.Nombre_Destinatario,
        ...
        res.status(500).json({
            success: false,
            message: 'Error al agregar el envío a la ruta.'
        });
    }
};
...
exports.confirmarEntregaCliente = async (req, res) => {
    try {
        ...
        if (!codigo || codigo.toString().trim() === '') {
            return res.status(400).json({
                success: false,
                message: 'Debe ingresar el código de confirmación del cliente.'
            });
        }
        ...
        if (envio.id_repartidor !== currentUserId) {
            return res.status(403).json({
                success: false,
                message: 'No tienes permiso para entregar este envío.'
            });
        }

        const resultado = await CodigoConfirmacion.validarYUsar(codigo.toString().trim(), id, currentUserId);
        ...
        if (resultado.tipo !== 'CLIENTE_CHOFER') {
            return res.status(400).json({
                success: false,
                message: 'El código no es válido para confirmar la entrega. Solicite al cliente que genere el código en la página de rastreo.'
            });
        }
        ...
        await logger.logAction(currentUserId, 'CONFIRMAR_ENTREGA_MOVIL', `Entrega confirmada para envío ${id} (app móvil)`);
        ...
        if (error.code === 'ER_NO_SUCH_TABLE') {
            return res.status(503).json({
                success: false,
                message: 'El sistema de códigos no está configurado.'
            });
        }
