...
class ApiService {
  ...
  // Para Windows/Web: http://localhost:3001/mobile/api
  ...
  // Obtener cookies del navegador (solo en web)
  String? _getBrowserCookies() {
    try {
  ...
  Map<String, String> get headers {
    ...
    // En web, obtener cookies del navegador
    final browserCookies = _getBrowserCookies();
    if (browserCookies != null && browserCookies.isNotEmpty) {
  ...
  // Helper para usar fetch de JavaScript (mejor manejo de cookies)
  Future<Map<String, dynamic>> _fetchWithCookies(String url, {
    String method = 'GET',
    Map<String, String>? headers,
    String? body,
  }) async {
    try {
      final jsHeaders = js.JsObject.jsify({
        'Content-Type': headers?['Content-Type'] ?? 'application/json',
        ...?headers,
      });
      
      final fetchOptions = js.JsObject.jsify({
        'method': method,
        'headers': jsHeaders,
      ...
      print('Realizando petición fetch a: $url con credentials: include');
      
      final promise = js.context.callMethod('fetch', [url, fetchOptions]);
      final response = await _promiseToFuture<js.JsObject>(promise);
      
      final status = response['status'] as int;
      print('Respuesta recibida con status: $status');
      
      final textPromise = response.callMethod('text');
      final responseText = await _promiseToFuture<String>(textPromise);
      
      // Verificar cookies después de la respuesta
      final cookiesAfter = _getBrowserCookies();
      print('Cookies después de la petición: $cookiesAfter');
      ...
      print('Error en _fetchWithCookies: $e');
      rethrow;
    }
  }
  
  Future<T> _promiseToFuture<T>(dynamic promise) async {
    final completer = Completer<T>();
    
    // Crear callbacks que manejen correctamente los resultados
    final thenFunc = (result) {
    ...
    promise.callMethod('then', [js.allowInterop(thenFunc)]);
    promise.callMethod('catch', [js.allowInterop(catchFunc)]);
  ...
  Future<Map<String, dynamic>> login(String username, String password) async {
    try {
      ...
      // En Flutter web, usar fetch de JavaScript para mejor manejo de cookies
      try {
        final result = await _fetchWithCookies(
          url,
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
          },
          body: 'nombre_usuario=$username&password=$password',
        );

        if (result['status'] == 200) {
          // Verificar cookies después del login
          final cookiesAfterLogin = _getBrowserCookies();
          print('Login exitoso - cookies del navegador: $cookiesAfterLogin');
          
          final responseText = result['body'] as String;
          return json.decode(responseText);
        } else {
          final responseText = result['body'] as String;
          final error = json.decode(responseText);
  ...
  Future<List<Envio>> getEnvios() async {
    try {
      ...
      try {
        ...
        // Fallback a http normal si dart:html no está disponible
        await _loadCookie();
  ...
  Future<EnvioDetalle> getEnvioDetalle(int id) async {
    try {
      ...
      try {
        ...
        if (browserCookies != null && browserCookies.isNotEmpty) {
          requestHeaders['Cookie'] = browserCookies;
        }
    ...
    try {
      ...
      try {
        ...
        if (response.status == 200) {
          ...
          final responseText = response.responseText ?? '{"message": "Error desconocido"}';
          final error = json.decode(responseText);
